...
const addResult = async (req, res) => {
  try {
    ...
    const DiemTK = parseFloat((diemArray.reduce((a, b) => a + b, 0) / diemArray.length).toFixed(1))
  ...
  try {
    ...
    if (updatedResult.TrangThai === 'Đã lấy') {
      if (!account.ChungChiDaNhan.includes(certificateId)) {
        ...
        if (account.ChungChiDaNhan.includes(certificateId)) {
          account.ChungChiDaNhan = account.ChungChiDaNhan.filter(id => String(id) !== certificateId);
          await account.save();
        }
      }
...
const deleteResult = async (req, res) => {
  try {
    ...
    if (result.TrangThai === 'Đã lấy') {
      await Account.findByIdAndUpdate(result.IDNguoiDung, {
